...
class MicroscopyPanel extends Component {
  ...
  saveFunction = async SeriesDescription => {
    const { microscopyManager, studyInstanceUID } = this;
    const { onSaveComplete, onRejectComplete } = this.props;
  ...
  render() {
    ...
    const onDeleteCurrentSRHandler = async () => {
      try {
        ...
        const rejectionFunction = dcm4cheeReject(study.wadoRoot);
        await rejectionFunction.series(
